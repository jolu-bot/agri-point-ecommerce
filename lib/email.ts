...
/**
 * Service Email utilisant Gmail/SMTP
 * Configuration requise dans .env.local:
 * - EMAIL_USER: Adresse email Gmail
 * - EMAIL_PASSWORD: App Password Gmail (pas le mot de passe du compte)
 */
...
function getTransporter() {
  ...
  if (!emailUser || !emailPassword) {
    console.warn('⚠️  Email non configuré. Définissez EMAIL_USER et EMAIL_PASS dans .env.local');
    return null;
  }
...
export async function sendEmail(options: EmailOptions): Promise<boolean> {
  try {
    ...
    if (!transport) {
      console.warn('❌ Transporter email non configuré');
      return false;
    }
